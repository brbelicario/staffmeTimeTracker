...
- Required setup modal for contract dates, duty window, adherence days, and the minimum-fee/excess-rate pay estimate; advanced rules and optional Beta Tracker identity linking remain in Tracker Settings
...
The dashboard starts empty. It does not create demo rows. Data appears only after the helper reads the visible StaffMe “Hourly AHT (Last 5 Days)” table and sends those rows to the dashboard. Accounts without contract dates must complete the required setup modal before using the dashboard; incomplete accounts have no duty window or adherence classification until setup is saved. Each account has its own saved profile and can be used from another device; an optional linked SM worker identity prevents rows from different SM accounts in the same browser from being mixed.
